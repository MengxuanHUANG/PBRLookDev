...
namespace PBRLookDev
{
	...
	bool PerspectiveCameraController::OnEvent(MyCore::Event& event)
	{
		EventDispatcher dispatcher(event);
		return dispatcher.Dispatch<MyCore::MouseMovedEvent>(std::bind(&PerspectiveCameraController::OnMouseMoved, this, std::placeholders::_1));
	}

	bool PerspectiveCameraController::OnMouseMoved(MyCore::MouseMovedEvent& mouse_moved_event)
	{
		WindowsWindow* window = WindowsWindow::GetWindow();
		glm::vec2 newPos = glm::vec2(mouse_moved_event.GetX(), mouse_moved_event.GetY());
